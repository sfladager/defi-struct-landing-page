...
export function NetworkStatus({ className }: any) {
  ...
  const classes = classNames(className, 'NetworkStatus', 'bg-gray-100 z-10 flex shadow-md items-center rounded-full overflow-hidden')
  const classesBadge = classNames(
    ...
    `text-${GetNetworkColor(network.chain?.network)}-700 py-2`
  )
  ...
  return (
    <div className={classes}>
      ...
      {explorerUrl && (
        <LinkComponent href={explorerUrl} className="mx-3 text-xs">
          <>#{block.data}</>
        </LinkComponent>
      )}
      {!explorerUrl && <span className="mx-3 text-xs"># {block.data ? '0' : ''}</span>}
    </div>
  )
}